...
/*
const { hasToBeAbsolute, hasToBeFile, hasToBeReadFile, hasToBeMd } = require('./index.js');
...
const mdLinks = (pathUser, options) => {
  return new Promise((resolve, reject) => {
    ...
    const absolutePath = hasToBeAbsolute(pathUser);
    let mdFiles = [];

    const stats = hasToBeFile(pathUser);
    const beMd = hasToBeMd(pathUser);

    if (stats === "Directorio") {
      mdFiles = hasToBeReadFile(pathUser);
    } else if (stats === "Archivo" && beMd == true) {
      mdFiles = hasToBeReadFile(pathUser)
        .then((mdFiles) => {
          return mdFiles;
        })
      ...
      return reject("La ruta especificada no es un directorio ni un archivo .md válido.");
